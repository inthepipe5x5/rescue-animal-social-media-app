...
class TestJSONFlattening(unittest.TestCase):
    def setUp(self):
        self.original_dict = {
            "organization": {
                ...
                "distance": None,
                "_links": {
                    "self": {"href": "/v2/organizations/nj333"},
                    "animals": {"href": "/v2/animals?organization=nj333"},
                },
            }
        }
    ...
    def test_flattened_keys(self):
        for key in self.new_keys:
            self.assertIn(
                key,
                self.original_dict["organization"]
                or self.original_dict["organization"]["address"]
                or self.original_dict["organization"]["adoption"]
                or self.original_dict["organization"]["social_media"]
                or {"photo": True, "self_href": True, "animals_href": True},
    ...
    def test_removed_keys(self):
        ...
        for key in removed_keys:
            self.assertNotIn(
                key, self.new_keys, f"Removed key '{key}' found in new keys"
            )

    def test_changed_keys(self, changed_keys: dict[str, str]):

        for old_key, new_key in changed_keys.items():
            self.assertIn(
                new_key, self.new_keys, f"Changed key '{new_key}' not found in new keys"
            )
            self.assertNotIn(
                old_key.split(".")[-1],
                self.new_keys,
                f"Old key '{old_key.split('.')[-1]}' found in new keys",
            )
